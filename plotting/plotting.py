    ...
    for soln_type in solution_types:
        ...
        for i in np.arange(len(timestep_vals)):
            ...
            if slice=='y':
                ...
                if obs and show_obs:
                    if shape==0:
                        ax.add_patch(plt.Circle((params[0], params[2]), params[3], fill=fill, color='k'))
                    elif shape==1:
                        x0 = np.array([params[0],params[2]])
                        h = params[2]
                        r = params[3]
                        ax.add_patch(plt.Circle(x0, r, fill=0, color='k'))
                        ax.add_patch(plt.Rectangle([params[0]-r,0],2*r,h,fill=fill, color='k'))
            elif slice=='z':
                ...
                if obs and show_obs:
                    if shape==0:
                        ax.add_patch(plt.Circle((params[0], params[1]), params[3], fill=fill, color='k'))
                    elif shape==1:
                        x0 = np.array([params[0],params[1]])
                        h = params[2]
                        r = params[3]
                        ax.add_patch(plt.Circle(x0, r, fill=fill, color='k')) # looks like a circle from top down
...
def make_gifs(timestamp, solution_types, interval, slice, show_obs):
    ...
    for soln_type in solution_types:
        ...
        if obs and show_obs:
            if shape==0:
                xy.add_patch(plt.Circle((params[0], params[1]), params[3], fill=fill, color='k'))
                xz.add_patch(plt.Circle((params[0], params[2]), params[3], fill=fill, color='k'))
            elif shape==1:
                x0 = np.array([params[0],params[2]])
                h = params[2]
                r = params[3]
                xz.add_patch(plt.Circle(x0, r, fill=fill, color='k'))
                xz.add_patch(plt.Rectangle([params[0]-r,0],2*r,h,fill=fill, color='k'))
                xy.add_patch(plt.Circle([params[0],params[1]], r, fill=fill, color='k'))
